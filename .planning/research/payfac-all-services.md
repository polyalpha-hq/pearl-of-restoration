# US accept-payments — все найденные сервисы

Одна таблица: главный каталог + ISO с живой публичной заявкой.

Путь: **моментально** = сам открыл аккаунт · **заявка** = публичная KYB-форма · **триал→заявка** = SaaS-триал, платежи из продукта · **демо** = sales/демо, потом заявка · **BYO** = свой Square/Helcim.

`*` = продукт старше 2020, путь открытый. Keyed ACH `да` = плательщик сам вбивает routing+account.
